# Voice Packs TODO

Game character voices to add as SC Commander voice packs.

## Tier 1 — Perfect Fit (military AI / advisor)

- [ ] **EVA** — Command & Conquer (same concept: military AI status updates)
- [ ] **GLaDOS** — Portal (passive-aggressive task commentary)
- [ ] **Cortana** — Halo (AI companion tactical briefings)
- [ ] **HEV Suit** — Half-Life (pure status reports: "Warning. Vital signs critical.")
- [ ] **EDI** — Mass Effect (ship AI, dry delivery)
- [ ] **SHODAN** — System Shock (menacing AI overlord)
- [ ] **VEGA** — Doom 2016+ (calm AI during chaos)
- [ ] **Ghost** — Destiny (task companion, "Guardian down" energy)

## Tier 2 — Iconic, Easy to Adapt

- [ ] **Colonel Campbell** — Metal Gear Solid (mission briefing style)
- [ ] **Deckard Cain** — Diablo ("Stay a while and listen" sage advisor)
- [ ] **Central / Bradford** — XCOM (tactical urgency)
- [ ] **Sean Bean narrator** — Civilization VI (gravitas, session complete moments)
- [ ] **Arthas / Kerrigan** — Warcraft III (peon-ping infrastructure exists)
- [ ] **Announcers** — Dota 2 / LoL ("Double kill!" energy for completions)
- [ ] **Athena** — Overwatch (clean AI announcer)

## Tier 3 — Fun / Novelty

- [ ] **Papyrus** — Undertale (comedic contrast)
- [ ] **PDA** — Subnautica ("Oxygen" warnings map to resource.limit perfectly)
- [ ] **Ship AI** — Dead Space (horror-toned errors)
- [ ] **Mr. New Vegas / Three Dog** — Fallout (radio DJ announcing progress)
